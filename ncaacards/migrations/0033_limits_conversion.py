    ...
    def forwards(self, orm):
        with transaction.commit_on_success():
            for game in orm.NcaaGame.objects.all():
                for user_team in orm.UserTeam.objects.filter(entry__game=game):
                    user_team.count -= game.position_limit
                    user_team.save()
                for entry in orm.UserEntry.objects.filter(game=game):
                    entry.extra_points -= game.points_limit
                    entry.save()
                    entry.update_score()
